...
//-----------------------------------------------------
    ...
    while(1){
        ...
        if (resetAmps==0){
            ...
            ESP_LOGI("main","%f",toyLift->maxFlatLoad);
        }
        ...
        updateAngles(IMU);
        calculateDynamicLoad(toyLift,IMU->pitch-pitchOffset,TOF->distance-1 );//calculating max load at current pitch/load height
        ...
        ESP_LOGI("main","pitch: %d | maxLoad: %f kg| currentLoad: %f kg| height: %dcm|UID: %02X %02X %02X %02X|err:%d|noise:%d",IMU->pitch-pitchOffset,toyLift->maxDynamicLoad,currentWeight,TOF->distance,uid[0], uid[1], uid[2], uid[3],error,noiseLevel);
        ...
        snprintf(toDisplay2, sizeof(toDisplay2), "c: %0.3f", currentWeight);
        ...
        //sending to MQTT
        char msg[128];
        snprintf(msg, sizeof(msg), "Driver:Matt,MaxWeight:%0.3f,CurrentWeight:%0.3f,Pitch:%d,LoadHeight:%d,score:%d,UID:%02X%02X%02X%02X", toyLift->maxDynamicLoad,currentWeight,IMU->pitch-pitchOffset,10,safetyScore,uid[0], uid[1], uid[2], uid[3]);
