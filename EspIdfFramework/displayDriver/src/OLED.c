...
void drawWeightChart(ssd1306_handle_t OLED,float weightPrecent){

    ssd1306_clear_display(OLED, false);
    ssd1306_display_bitmap(OLED, 120, 0, line_icon_32x32, 1, 32, false);

    float xIndex=120*(weightPrecent/100);
    float heightCount=0;
    for (int i=0;i<(int)xIndex;i++){
        heightCount++;
        float mapHeight=32*(heightCount/120);
        ssd1306_display_bitmap(OLED,i, 32-(int)mapHeight, line_icon_32x32, 1, (int)mapHeight, false);
        if (weightPrecent>75){
        ssd1306_display_bitmap(OLED, 0, 0, exclaim_bitmap8x16, 8, 16, false);
    }
