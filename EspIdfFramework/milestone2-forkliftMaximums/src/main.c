...
//-----------------------------------------------------
    ...
    while(1){
       ...
       // ESP_LOGI("main"," Left raw: %d || Right raw: %d",leftForkSensor->Weight,rightForkSensor->Weight);
        ESP_LOGI("main","pitch: %d | maxLoad: %f kg| currentLoad: %f kg| height: %dcm",IMU->pitch-2,toyLift->maxDynamicLoad,currentWeight,TOF->distance);
        ...
        //sending to MQTT
        char msg[128];
        snprintf(msg, sizeof(msg), "Driver:Matt,MaxWeight:%0.3f,CurrentWeight:%0.3f,Pitch:%d,LoadHeight:%d,score:%d", toyLift->maxDynamicLoad,currentWeight,IMU->pitch-2,10,safetyScore);
